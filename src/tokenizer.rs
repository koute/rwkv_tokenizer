...
impl Tokenizer {
    pub fn new(vocabulary_json: &str) -> Result<Self, TokenizerError> {
        let map: BTreeMap<u16, StrOrBytes> = serde_json::from_str(vocabulary_json)
            .map_err(TokenizerErrorKind::FailedToParseVocabulary)
            .map_err(TokenizerError)?;

        let list: Vec<(Vec<u8>, u16)> = map.into_iter().map(|(token, pattern)| {
    ...
    pub fn encode_into(&self, mut input: &[u8], output: &mut Vec<u16>) -> Result<(), TokenizerError> {
    ...
    pub fn decode_into(&self, tokens: &[u16], output: &mut Vec<u8>) -> Result<(), TokenizerError> {
        for &token in tokens {
            let bytes = self.token_index_to_bytes.get(token as usize)
